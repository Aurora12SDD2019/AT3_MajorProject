...
while play:  # the loop runs as long as play == True
    ...
    if game_over:
        ...
        game_window.blit(score_text,[(SCREENWIDTH - score_x)/2, -100+(SCREENHEIGHT - score_y)/2])
        hi_score_x,hi_score_y = font_over.size("Current high score: {}".format(hi_score))        
        score_text = font_over.render("Current high score: {}".format(hi_score),1,red, blue)
        game_window.blit(score_text,[(SCREENWIDTH - hi_score_x)/2, (SCREENHEIGHT - hi_score_y)/2])
